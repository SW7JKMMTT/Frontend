...
export class MapComponent {
    ...
    ngOnInit(){
        ...
        this.map.on('moveend', (e: MouseEvent) => { 
            this.test();
        });
        ...
        var car = new L.marker([0, 0], {icon: new_icon}).on('click', function() { alert('JA'); } );
        var start = new L.marker([this.data[0][0], this.data[0][1]], {icon: start_icon});

        this.map.addLayer(start);
        this.map.addLayer(firstpolyline);
        this.map.addLayer(car);

        Observable.interval(200).take(this.data.length - 1).map((x) => x + 1).subscribe((x) => {
